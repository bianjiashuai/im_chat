...
public class RedisUtil {
     ...
     * @return
     */
    public static UserSession getUserSession(String userParam) {
     ...
     * @return
     */
    public static UserSession getUserSession(String userParam, boolean hasPassword) {
        UserSession userSession = JSON.parseObject(JEDIS.get(IM_WS_PREFIX + userParam), UserSession.class);
        if (!hasPassword) {
    ...
    public static void writeOutlineMsg(String dstUserId, MessageResponsePacket packet) {
        String srcUserId = packet.getSrcUserId();
